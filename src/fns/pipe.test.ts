...
describe('pipe and pipeAsync', () => {
  ...
  describe("pipe", () => {
    ...
    it('should return in coorect order value', () => {
      const multThanInc = pipe(
        multBy2,
        increment,
      )
      const incThanMult = pipe(
        increment,
        multBy2,
      )
    ...
    it('should work with generics', () => {
      ...
      const changeAgeAndNameComposed = pipe(
        changeUserAge(10),
        changeNameToIlyia,
      )

      const changeNameAndAgeComposed = pipe<User[], User, User>(
        changeNameToIlyia,
        changeUserAge(10),
      )
  ...
  describe("pipeAsync", () => {
    ...
    it('should return promise and resolve it', async () => {
      const sumAsyncThanInc = pipeAsync(
        sumAsync,
        increment,
      )
